...
홈 화면에서 수집 기준일, 분류 탭, 기관, 출처, 문서 유형, 변경상태, 검색어로 필터링합니다. 법령의 공포일이 과거여도 해당 날짜 수집 API가 반환한 변경 건이면 그 날짜의 자료로 표시합니다. OpenAI API 키를 브라우저에 입력하면 현재 표시 중인 항목을 세션 안에서 요약할 수 있습니다. 배포 자동 수집 요약은 GitHub Actions의 `OPENAI_API_KEY` secret이 있을 때 수행됩니다.
