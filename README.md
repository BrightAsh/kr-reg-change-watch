...
- 국민참여입법센터 공개 API
  - 입법예고 XML: `ogLmPp.xml`
  - 행정예고 XML: `ptcpAdmPp.xml`
  - 법제처, 행정안전부, 기획재정부 기준으로 해당 날짜 예고 시작 자료 조회
- 대한민국 전자관보 API
...
- `LAWMAKING_OC`: 국민참여입법센터 정보공개 API ID. 없으면 `LAW_OPEN_API_OC`를 대신 사용
...
- 국민참여입법센터는 `.xml?OC=...` 형식의 공식 정보공개 API를 사용합니다. 단순 `/rest/ogLmPp` 호출은 HTML 안내 화면 또는 인증 오류만 받을 수 있어 수집에 쓰지 않습니다.
- 국민참여입법센터 응답이 `retMsg 401`이면 현재 ID가 해당 API에 승인되지 않은 상태입니다. 이때는 국민참여입법센터용 정보공개 API ID를 `LAWMAKING_OC` secret으로 별도 저장해야 합니다.
- 행정안전부와 기획재정부 게시판은 목록의 글 링크가 일반 `<a href>`가 아니라 자바스크립트 함수로 열리는 경우가 있어, 수집기가 게시글 ID를 추출해 상세 URL을 재구성합니다.
- 기획재정부 게시판 기본 도메인은 현재 응답이 안정적인 `https://mofe.go.kr`를 사용합니다. 필요하면 `MOEF_*_URL` 환경변수로 개별 게시판 URL을 바꿀 수 있습니다.
- 날짜별 비교는 수집 대상일의 바로 이전 스냅샷과 비교합니다. 과거 날짜를 나중에 강제 재수집해도 최신 날짜와 잘못 비교하지 않습니다.
