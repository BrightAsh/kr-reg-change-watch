...
| AI 브리핑 | 사용자가 OpenAI API 키를 입력하면 현재 목록 전체를 업무용 브리핑으로 요약합니다. |
...
AI 기능은 OpenAI GPT API 키를 사용자가 직접 입력했을 때만 동작합니다.

| 기능 | 입력 자료 | 결과 | 비용 |
| --- | --- | --- | --- |
| AI 브리핑 | 현재 화면에 표시된 여러 항목의 제목, 기관, 날짜, 본문 일부 | 선택 날짜 또는 필터 결과 전체를 업무용 브리핑으로 요약 | OpenAI API 사용량에 따라 비용 발생 |
| AI 요약 | 상세 화면의 수집 본문 | 개별 항목의 핵심 내용, 적용 대상, 확인할 업무 영향을 요약 | OpenAI API 사용량에 따라 비용 발생 |

AI 요약은 현재 앱이 이미 수집한 본문을 기준으로 동작합니다. 원문 URL을 GPT가 직접 웹 검색해서 읽는 방식은 기본 동작이 아닙니다. 원문 본문을 더 많이 확보할수록 AI 요약 품질도 좋아집니다.
