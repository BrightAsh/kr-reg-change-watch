...
import { parseArgs, readJson, rootDir, writeJson } from "./common";
...
const dataRoot = path.join(rootDir, "data", "data");
...
async function main() {
  ...
  const todayItems = todayFile ? await readJson<CollectedItem[]>(path.join(snapshotsDir, todayFile), []) : [];
  const yesterdayItems = yesterdayFile ? await readJson<CollectedItem[]>(path.join(snapshotsDir, yesterdayFile), []) : [];
  ...
  await writeJson(path.join(dataRoot, "diff.json"), result);
  console.log(`Data diff complete. Added: ${added.length}, changed: ${changed.length}, removed: ${removed.length}.`);
}
